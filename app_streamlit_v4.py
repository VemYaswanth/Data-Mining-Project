...
# app_streamlit_v3.py
# Omnichannel Retail Intelligence Dashboard (v3)
# Tabs: Dataset & Rules | Temporal Insights | Customer Segmentation | Smart Recommender
...
st.title("📈 Omnichannel Retail Intelligence Dashboard (v3)")
st.caption("Association rules + temporal trends + customer segmentation + recommender")
...
def detect_schema(df: pd.DataFrame) -> dict:
    ...
    # numeric
    for cand in ["finalamount", "final_amount", "total", "amount"]:
...
def build_transactions(df: pd.DataFrame, schema: dict) -> list:
    ...
    elif schema["customer"] and schema["date"]:
        # per customer-day basket
        try:
...
if choice != "Upload CSV" and upl is None:
    df = load_csv_safely("/mnt/data/" + choice)
else:
    df = pd.read_csv(upl) if upl is not None else pd.DataFrame()
...
# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["📊 Dataset & Rules", "🕒 Temporal Insights", "👥 Customer Segmentation", "🛍️ Smart Recommender"])
...
with tab4:
    ...
    # Ensure rules exist (from Tab1). If not, rebuild quickly on filtered data.
    if 'rules' not in locals() or rules.empty:
        tx2 = build_transactions(df2, schema)
        if tx2:
            enc2 = encode_transactions(tx2)
            fi2, _ = run_mining(enc2, "FP-Growth", 0.01, 3)
    ...
    if rules2.empty:
        st.warning("No rules available. Generate rules in the first tab or broaden filters.")
    else:
        # item picker
        try:
            all_items = sorted(set().union(*rules2['antecedents']).union(*rules2['consequents']))
        except Exception:
            # fallback if sets not present
        ...
        if selected:
            ...
            if picked_seg != "(none)" and not rfm_cache.empty and schema["customer"]:
            ...
            # Build recommendations from rules
            sub = rules2[rules2["antecedents"].apply(lambda s: set(selected).issubset(s))]
            if sub.empty:
                st.info("No direct rules for selected base product(s). Try selecting one item or lowering thresholds.")
            else:
                ...
                # Impact: revenue where rule applies
                if prod_col in df_seg.columns and schema["finalamount"] in df_seg.columns and schema["invoice"] in df_seg.columns:
                    df_seg[prod_col] = normalize_products(df_seg[prod_col])
                    by_inv = df_seg.groupby(schema["invoice"]).agg(
